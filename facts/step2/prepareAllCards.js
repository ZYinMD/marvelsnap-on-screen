...
for (const card of input) {
  ...
  if (card.name in output) throw new Error(`Duplicated card id: ${card.name}`);
  delete card.artists; // many cards have this field, we don't need
  output[card.name] = card;
}

const outputPath = new URL('../../ui/src/lib/facts/allCards.ts', import.meta.url);
const tsFile = `export const allCards = ${JSON.stringify(output)} as const;`;
fs.writeFile(outputPath, tsFile);
